...
重点:webpack打包大致流程
主要功能是：
把解析的所有模块变成一个个对象，然后通过唯一入口，去加载这些对象，依次实现递归的依赖关系，然后通过入口运行所有的模块（文件）
...
## 把模式改成'pruduction'后,css打包压缩,需要进入npm官网查找mini-css-extract-plugin，看看文档
...
外部配置：官网下载 .eslintrc.json文件，放到webpack.config.js同目录下
...
4）补充：针对图片小，就不去发送http请求图片，而是以base64（base64比源文件会大1/3）方式加载，
一般不直接用file-loader，而是用url-loader -D
